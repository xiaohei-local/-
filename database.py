...
def log_alert(video_source, track_id, action, action_cn, confidence, frame_idx):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO alerts (video_source, track_id, action, action_cn, confidence, frame_idx) VALUES (?,?,?,?,?,?)",
...
def get_recent_alerts(limit=50):
    conn = _connect()
    c = conn.cursor()
    c.execute("SELECT timestamp, track_id, action_cn, confidence FROM alerts ORDER BY id DESC LIMIT ?", (limit,))
    rows = c.fetchall()
    conn.close()
    return rows
...
def get_alert_stats(video_source=None):
    ...
    if video_source:
        c.execute(
            "SELECT action, action_cn, COUNT(*) FROM alerts WHERE video_source=? "
            "GROUP BY action ORDER BY COUNT(*) DESC",
            (video_source,),
        )
    else:
        c.execute("SELECT action, action_cn, COUNT(*) FROM alerts GROUP BY action ORDER BY COUNT(*) DESC")
    rows = c.fetchall()
    conn.close()
    return rows
