...
class BiLevelRoutingAttention(nn.Module):
    ...
    Partitions the feature map into regions, computes region-level affinity to route
    top-k relevant regions, then performs fine-grained token-to-token attention only
    within the routed regions.
    ...
    def forward(self, x):
        ...
        v_g = torch.gather(
            v.unsqueeze(2).expand(-1, -1, n_win * n_win, -1, -1, -1), 3, idx_r
        )

        # Token attention within routed regions
        q_t = q.reshape(B, n_win, rh, n_win, rw, self.num_heads, -1)
        q_t = q_t.permute(0, 1, 3, 5, 2, 4, 6).reshape(
            B, n_win * n_win, self.num_heads, rh * rw, -1
        )
