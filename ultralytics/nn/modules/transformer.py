...
class BiLevelRoutingAttention(nn.Module):
    """Bi-Level Routing Attention (BRA) for dynamic sparse attention.

    Partitions the feature map into regions, computes region-level affinity to route top-k relevant regions, then
    performs fine-grained token-to-token attention only within the routed regions.

    Attributes:
        dim (int): Total input channels.
        num_heads (int): Number of attention heads.
        n_win (int): Number of region partitions per spatial dimension.
        topk (int): Number of top regions to attend to.
        qk_dim (int): Dimension per head for Q/K projections.
        scale (float): Scaling factor for attention logits.
    """
    ...
    def forward(self, x):
        ...
        """
        B, C, H, W = x.shape
        n_win = min(self.n_win, H, W)
        pad_h = (n_win - H % n_win) % n_win
        pad_w = (n_win - W % n_win) % n_win
        if pad_h > 0 or pad_w > 0:
            x = F.pad(x, (0, pad_w, 0, pad_h))
        B, C, Hp, Wp = x.shape
        assert Hp % n_win == 0 and Wp % n_win == 0

        q = self.q(x.permute(0, 2, 3, 1))  # (B, H, W, C)
        kv = self.kv(x.permute(0, 2, 3, 1))  # (B, H, W, 2C)
        k, v = kv.chunk(2, dim=-1)

        # Region partition
        rh, rw = Hp // n_win, Wp // n_win
        q_r = q.reshape(B, n_win, rh, n_win, rw, self.num_heads, -1)
        q_r = q_r.permute(0, 1, 3, 5, 2, 4, 6)  # (B, n_win, n_win, n_heads, rh, rw, head_dim)
        q_r = q_r.flatten(4, 5).mean(dim=4)  # (B, n_win, n_win, n_heads, head_dim)

        k_r = k.reshape(B, n_win, rh, n_win, rw, self.num_heads, -1)
        k_r = k_r.permute(0, 1, 3, 5, 2, 4, 6).flatten(4, 5).mean(dim=4)

        # Region affinity: (B, n_heads, n_win², n_win²)
        attn_r = torch.einsum("bxyhc,buvhc->bhxyuv", q_r, k_r) * self.scale
        attn_r = attn_r.reshape(B, self.num_heads, n_win * n_win, n_win * n_win)

        # Top-k routing
        topk = min(self.topk, n_win * n_win)
        with torch.no_grad():
            _, idx_r = torch.topk(attn_r, k=topk, dim=-1)  # (B, n_heads, n_win², topk)

        # Gather key/value tokens from routed regions
        k = k.reshape(B, n_win, rh, n_win, rw, self.num_heads, -1)
        k = k.permute(0, 1, 3, 5, 2, 4, 6).reshape(B, n_win * n_win, self.num_heads, rh * rw, -1)
        v = v.reshape(B, n_win, rh, n_win, rw, self.num_heads, -1)
        v = v.permute(0, 1, 3, 5, 2, 4, 6).reshape(B, n_win * n_win, self.num_heads, rh * rw, -1)

        idx_r = idx_r.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, -1, rh * rw, C // self.num_heads)
        k_g = torch.gather(
            k.unsqueeze(2).expand(-1, -1, n_win * n_win, -1, -1, -1), 3, idx_r
        )  # (B, n_win², n_heads, topk, rh*rw, head_dim)
        v_g = torch.gather(v.unsqueeze(2).expand(-1, -1, n_win * n_win, -1, -1, -1), 3, idx_r)

        # Token attention within routed regions
        q_t = q.reshape(B, n_win, rh, n_win, rw, self.num_heads, -1)
        q_t = q_t.permute(0, 1, 3, 5, 2, 4, 6).reshape(B, n_win * n_win, self.num_heads, rh * rw, -1)
        attn_t = torch.einsum("brhtd,brhTkd->brhtTk", q_t, k_g) * self.scale
        attn_t = attn_t.softmax(dim=-1)
        out = torch.einsum("brhtTk,brhTkd->brhtd", attn_t, v_g)
        out = out.reshape(B, n_win, n_win, self.num_heads, rh, rw, -1)
        out = out.permute(0, 1, 4, 2, 5, 3, 6).reshape(B, Hp, Wp, C)
        out = self.proj(out)
...
class BiFormerBlock(nn.Module):
    """Transformer block using Bi-Level Routing Attention with a feed-forward network.

    Args:
        dim (int): Input channels.
        num_heads (int): Number of attention heads.
        n_win (int): Number of region partitions per spatial dimension (default 7).
        topk (int): Number of top regions to attend to (default 4).
        mlp_ratio (float): Expansion ratio for the FFN hidden layer.
        drop (float): Dropout rate.
    """

    def __init__(self, dim, num_heads=8, n_win=7, topk=4, mlp_ratio=4.0, drop=0.0):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = BiLevelRoutingAttention(dim, num_heads, n_win, topk)
    ...
    def forward(self, x):
        """Args: x (B,C,H,W). Returns: (B,C,H,W).
        """
        shortcut = x
        x = x.permute(0, 2, 3, 1)  # B,H,W,C
        x = shortcut + self.attn(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
